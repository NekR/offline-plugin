...
var OfflinePlugin = (function () {
  function OfflinePlugin(options) {
    ...
    if (this.options.scope) {
      compilation.warnings.push(new Error('OfflinePlugin: `scope` option is deprecated, use `publicPath` instead'));

      if (this.publicPath) {
        compilation.warnings.push(new Error('OfflinePlugin: `publicPath` is used with deprecated `scope` option, `scope` is ignored'));
      } else {
    ...
    if (this.relativePaths && this.publicPath) {
      compilation.warnings.push(new Error('OfflinePlugin: publicPath is used in conjunction with relativePaths\n' + 'OfflinePlugin: publicPath was set by OfflinePlugin to empty string'));
