...
var OfflinePlugin = (function () {
  function OfflinePlugin(options) {
    ...
    if (this.relativePaths && this.cdnPath) {
      this.warnings.push(new Error('OfflinePlugin: cdnPath is used in conjunction with relativePaths,\n' + 'cdnPath was set by the OfflinePlugin to empty string'));
  ...
  _createClass(OfflinePlugin, [{
    ...
    value: function validatePaths(assets) {
      ...
      return assets.map(this.rewrite).filter(function (asset) {
        ...
        if (key === '/') {
          return _this5.publicPath + key;
        }

        return _this5.cdnPath + _this5.publicPath + key.replace(/^\//, '');
      });
    }
  }, {
