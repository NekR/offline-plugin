...
export default class OfflinePlugin {
  ...
  apply(compiler) {
    ...
    compiler.plugin('normal-module-factory', nmf => {
      nmf.plugin('after-resolve', (result, callback) => {
        ...
        result.loaders.push(path.join(__dirname, 'misc/runtime-loader.js') + '?' + JSON.stringify(data));
    ...
    compiler.plugin('emit', (compilation, callback) => {
      ...
      if (compilation.fileDependencies.indexOf(runtimeTemplatePath) === -1 && !this.__tests.ignoreRuntime) {
      ...
      try {
        ...
        this.hash = loaderUtils.getHashDigest(Object.keys(this.hashesMap).join(''), 'sha1');
  ...
  setAssets(compilation) {
    ...
    if (caches === 'all') {
      ...
      const handledCaches = ['main', 'additional', 'optional'].reduce((result, key) => {
        ...
        cache.some(cacheKey => {
          ...
          if (!isAbsoluteURL(cacheKey) && cacheKey[0] !== '/' && cacheKey.indexOf('./') !== 0 && (magic = hasMagic(cacheKey))) {
            let matched;
      ...
      if (restSection && assets.length) {
        handledCaches[restSection] = handledCaches[restSection].concat(this.validatePaths(assets));
      }

      if (externalsSection && externals.length) {
        handledCaches[externalsSection] = handledCaches[externalsSection].concat(this.validatePaths(externals));
      }
  ...
  setHashesMap(compilation) {
    ...
    Object.keys(compilation.assets).forEach(key => {
      ...
      if (typeof validatedPath !== 'string' || this.assets.indexOf(validatedPath) === -1) return;

      const hash = loaderUtils.getHashDigest(compilation.assets[key].source(), 'sha1');
  ...
  validatePaths(assets) {
    return assets.map(this.rewrite).filter(asset => !!asset).map(key => {
