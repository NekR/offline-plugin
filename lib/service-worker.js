...
var ServiceWorker = (function () {
  ...
  _createClass(ServiceWorker, [{
    ...
    value: function addEntry(plugin, compilation, compiler) {
      ...
      if (this.minify === true) {
        if (_miscGetUglifyPlugin.makeUglifyJsPlugin == null) {
        ...
        (0, _miscGetUglifyPlugin.makeUglifyJsPlugin)(options).apply(childCompiler);
      } else if ((this.minify !== false || optimization.minimize) && _miscGetUglifyPlugin.makeUglifyJsPlugin) {
        ...
        var added = (optimization.minimize && optimization.minimizer || []).concat(compiler.options.plugins || []).some(function (plugin) {
          if ((0, _miscGetUglifyPlugin.isInstanceOfUglifyJsPlugin)(plugin)) {
    ...
    value: function apply(plugin, compilation, compiler) {
      ...
      if (typeof this.minify === 'boolean') {
        ...
        minify = _miscGetUglifyPlugin.makeUglifyJsPlugin && (!!(compiler.options.optimization && compiler.options.optimization.minimize) || !!(compiler.options.plugins || []).some(function (plugin) {
          return (0, _miscGetUglifyPlugin.isInstanceOfUglifyJsPlugin)(plugin);
        }));
      }
