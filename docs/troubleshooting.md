...
* **The ServiceWorker is not being updated.** Make sure the ServiceWorker (`sw.js`) is not being cached by the server (`Cache-Control: no-cache, max-age=0, no-store, must-revalidate`) and that it has a deterministic name (no hashing).
