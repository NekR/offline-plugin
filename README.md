...
Allows you to specify _external_ assets (assets which aren't generated by webpack) that should be included in the cache. If you don't change the `caches` configuration option then it should be enough to simply add _external_ assets to the cache. For other details and more advanced use cases see [`caches` docs](docs/caches.md).

> Default: `null`  
> **Example value:** `['fonts/roboto.woff']`

#### `excludes: Array<string | globs_pattern>`
Excludes matched assets from being added to the cache. Exclusion is performed before _rewrite_ happens.  
...
When set to `true`, all cached asset paths are generated relatively to `ServiceWorker` file or the `AppCache` folder location respectively.  
...
* `entry`: `string`. Relative or absolute path to the file which will be used as the `ServiceWorker` entry/bootstrapping. Useful to implement additional features or handlers for Service Worker events.  
...
* `navigateFallbackURL`: `string`. The URL that should be returned from the cache when a requested navigation URL isn't available within the cache. Similar to the `AppCache.FALLBACK` option.  
