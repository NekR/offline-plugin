...
Allows to specify _external_ assets (assets which aren't generated by webpack) to be cached. If you don't change `caches` option then this should be enough to add _external_ assets to the cache. For other details and more advanced use cases see [`caches` docs](docs/caches.md).

> Default: `null`  
> **Example value:** `['fonts/roboto.woff']`

#### `excludes: Array<string | globs_pattern>`
Excludes matched assets from being added to the caches. Exclusion is performed before _rewrite_ happens.  
...
When set to `true`, all cache asset paths are generated relatively to `ServiceWorker` file or `AppCache` folder location respectively.  
